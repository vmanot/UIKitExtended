...
open class UICarouselFlowLayout: UICollectionViewFlowLayout {
    ...
    override open func targetContentOffset(forProposedContentOffset proposedContentOffset: CGPoint, withScrollingVelocity velocity: CGPoint) -> CGPoint {
        guard let collectionView = collectionView, !collectionView.isPagingEnabled, let layoutAttributes = layoutAttributesForElements(in: collectionView.bounds)
            else {
        ...
        if isHorizontal {
            let closest = layoutAttributes
                .min { abs($0.center.x - proposedContentOffsetCenterOrigin) < abs($1.center.x - proposedContentOffsetCenterOrigin) } ?? UICollectionViewLayoutAttributes()
            
            return .init(x: floor(closest.center.x - midSide), y: proposedContentOffset.y)
        } else {
            let closest = layoutAttributes
                .min { abs($0.center.y - proposedContentOffsetCenterOrigin) < abs($1.center.y - proposedContentOffsetCenterOrigin) } ?? UICollectionViewLayoutAttributes()
            
            return .init(x: proposedContentOffset.x, y: floor(closest.center.y - midSide))
        }
    }
}
